...
RUN wget -q -O /usr/local/share/mise/data/installs/latexindent https://github.com/cmhughes/latexindent.pl/releases/download/V4.0/latexindent-linux && \
    ...
    wget -O codelldb.zip https://github.com/vadimcn/codelldb/releases/download/v1.12.1/codelldb-linux-x64.vsix && \
    ...
    rm -rf glsl_analyzer.zip bin

# AI
ENV PATH=/root/.local/bin:$PATH
ENV PATH=/root/.opencode/bin:$PATH
RUN curl -fsSL https://opencode.ai/install | HOME=/root bash
RUN curl https://cursor.com/install -fsS | HOME=/root bash
RUN curl -fsSL https://gh.io/copilot-install | HOME=/root bash
...
RUN mkdir -p /etc/xdg && cd /etc/xdg && \
    git clone --depth 1 --single-branch --branch container https://github.com/dddddjent/Nvim-configuration.git nvim && \
    rm -rf nvim/bin nvim/fonts && \
