---@brief
---
--- gopls config, based on nvim-lspconfig's default, but with `go` resolved via
--- mise (so Neovim itself doesn't need `go` on PATH).
...
local go_bin = MISE_DEP_DIR .. "/go/latest/bin"
local go_cmd = go_bin .. "/go"
...
local function identify_go_dir(custom_args, on_complete)
    local cmd = { go_cmd, "env", custom_args.envvar_id }

    if not path_exists(go_cmd) then
    ...
    vim.system(cmd, { text = true }, function(output)
        ...
        if output.code == 0 and res ~= "" then
            ...
            -- Keep the warning behavior from nvim-lspconfig, but avoid hard errors.
